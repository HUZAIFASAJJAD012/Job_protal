...
const UserChat = () => {
  ...
  useEffect(() => {
    if (UserInfo?.id) {
      console.log('UserChat: Joining socket room for user:', UserInfo.id);
  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        ...
        // Process chats for UI display using the new backend data
        const processedChats = await Promise.all(data.map(async (chat) => {
          try {
            ...
            const profilePicture = recipientId ? await fetchUserProfile(recipientId) : null;
              
            return {
              id: chat._id,
              name: chat.recipientName || "Unknown User", 
              message: chat.lastMessage ? chat.lastMessage.content : "Start a conversation",
              time: chat.lastMessage ? getTimeAgo(chat.lastMessage.timestamp) : getTimeAgo(chat.createdAt),
              ...
              read: chat.lastMessage && chat.lastMessage.sender && chat.lastMessage.sender.toString() === UserInfo.id.toString(),
              members: chat.members || [],
              lastMessageTime: chat.lastMessage ? chat.lastMessage.timestamp : chat.createdAt,
        ...
        if (selectedUserId) {
          const existingChat = data.find((chat) =>
            chat.members && chat.members.includes(selectedUserId)
          );
  ...
  const createNewChat = async (userId) => {
    ...
    try {
      ...
      setConversations(prev => [...prev, newChatData]);
  ...
  const handleBackToList = () => {
    ...
    setActiveChat(null); // Add this line - clear active cha
  };

  // Fetch messages when a chat is selected and mark as read
  ...
  useEffect(() => {
    const handleReceiveMessage = (message) => {
      console.log('UserChat: Received message:', message); // Add this for debugging
  ...
  console.log('Current UserInfo.id:', UserInfo.id);
      ...
      if (activeChat === message.conversationId) {
        console.log('UserChat: Adding message to active chat:', activeChat);
        setMessages((prev) => {
        ...
        console.log('UserChat: Message not for active chat. Active:', activeChat, 'Message chat:', message.conversationId);
      }
      
      // Update chats list with proper unread count
...
unreadCount: isUnread ? (conv.unreadCount || 0) + 1 : 0,
    ...
    const handleMessageRead = (data) => {
      console.log('UserChat: Message read receipt:', data);
  ...
  const sendMessage = async () => {
    ...
    if (!chat || !chat.members) return console.error("Chat not found or invalid");
    ...
    try {
      // Send via socket
      console.log('UserChat: Sending message:', messageData);
      ...
      console.log('UserChat: Message sent via socket:', messageData);
    } catch (error) {
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          <div className="space-y-1">
                            <h3 className="text-sm font-medium">{chat.name || "Unknown User"}</h3>
